...
const taskModal = document.querySelector(".TaskModalBody");
...
const loadInitialData = () => {
  ...
  if (localStorageCopy) state.taskSet = localStorageCopy.tasks;

  state.taskSet.map((cardDate) => {
...
// when user clicks open task
const openTask = (e) => {
  // pop up the current one
  if (!e) e = window.event;

  // find the crt card opened
  const getTask = state.taskSet.find(({ id }) => id === e.target.id);
...
const deleteTask = (e) => {
  ...
  const removeTask = state.taskSet.filter(({ id }) => id !== targetID);
  // console.log(removeTask);

  state.taskSet = removeTask;
  updateLocalStorage();
